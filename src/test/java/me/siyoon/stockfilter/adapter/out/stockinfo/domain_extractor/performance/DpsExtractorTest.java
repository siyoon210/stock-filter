...
class DpsExtractorTest {
    ...
    void dps_test(Period period, DPS expected) {
        ...
        // when
        DPS actual = DpsExtractor.dps(crawledData, period);
    ...
    private static Stream<Arguments> dps_test() {
        return Stream.of(
                Arguments.of(Period.TWO_YEARS_AGO, DPS.from(200.0)),
                Arguments.of(Period.LAST_YEAR, DPS.from(300.0)),
                Arguments.of(Period.THIS_YEAR_EXPECTED, DPS.from(300.0)),
                Arguments.of(Period.FOUR_QUARTERS_AGO, DPS.from(300.0)),
